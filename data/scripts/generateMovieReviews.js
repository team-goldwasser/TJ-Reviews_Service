...
fs.readFile('data/movieIDTitle.json', 'utf-8', (err, data) => {
  if (err) {
    ...
    movies.forEach ( (movie) => {
      var counter = 0;
      var numberOfReviews = getRandomInt(10);
      while (counter < numberOfReviews) {
        ...
        movieReview.liked = getRandomInt(2);
        counter++;
        i++;
        movieData.push(movieReview);
      }
    });
    fs.writeFile('data/movieReviews.json', JSON.stringify(movieData, null, 2), 'utf-8', (err) => {
      if (err) {
